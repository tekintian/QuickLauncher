...
public enum AppType: String, Codable {
    ...
    var localizedDescription: String {
        switch self {
        case .terminal:
            return NSLocalizedString("Terminal", comment: "Terminal app type")
        case .editor:
            return NSLocalizedString("Editor", comment: "Editor app type")
        }
    }
}
    ...
    public func openOutsideSandbox() throws {
        ...
        func excute(_ source: String) throws {
            guard let script = NSAppleScript(source: source) else {
                throw QuickLauncherError.cannotCreateAppleScript(source)
            }
            var error: NSDictionary?
            script.executeAndReturnError(&error)
            if error != nil {
                throw QuickLauncherError.cannotAccessApp(self.name)
            }
        }
...
//                }
    ...
    public func openInSandbox(_ urls: [URL]) throws {
        ...
        case .terminal:
            ...
            // script
            guard let scriptURL = ScriptManager.shared.getScriptURL(with: AppConfiguration.Scripts.generalScript) else { return }
