...
/// Install AppleScripts to $HOME/Library/Application Scripts/cn.tekin.app.QuickLauncherFinderExtension
func checkScripts() throws {
    guard var scriptFolderPath = try? FileManager.default.url(for: .applicationScriptsDirectory, in: .userDomainMask, appropriateFor: nil, create: true) else {
        throw QuickLauncherError.cannotAccessPath("$HOME/Library/Application Scripts/cn.tekin.app.QuickLauncher")
    }
    scriptFolderPath.deleteLastPathComponent()
    let finderExScriptPath = scriptFolderPath.appendingPathComponent(Constants.Id.FinderExtension, isDirectory: true)
